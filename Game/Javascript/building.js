// Constructors
function building(buildingName,realName,num,cost,land,isDisplayed){
	...
	this.buy = function()
	{
		// Check if user has enough money and territory to buy a building
		if (money >= this.cost && usedTerritory + this.land <= territory) {
			money -= this.cost;
			usedTerritory += this.land;
	...
	this.sell = function()
	{
		// Check if there are more than zero buildings
		if (this.number > 0)
		{
			this.number = this.number - 1;
		}
...
function buyBuilding(arrayIndex) {
	...
	if (money >= buildingArray[arrayIndex].cost && usedTerritory + buildingArray[arrayIndex].land <= territory) {
		money -= buildingArray[arrayIndex].cost;
